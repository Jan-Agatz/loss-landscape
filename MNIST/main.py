...
if __name__ == '__main__':
    ...
    if not os.path.exists('MNIST/trained_nets/' + save_folder):
        os.makedirs('MNIST/trained_nets/' + save_folder)

    f = open('MNIST/trained_nets/' + save_folder + '/log.out', 'a', 1)

    trainloader, testloader = dataloader.get_data_loaders(args)

    if args.label_corrupt_prob and not args.resume_model:
        torch.save(trainloader, 'MNIST/trained_nets/' + save_folder + '/trainloader.dat')
        torch.save(testloader, 'MNIST/trained_nets/' + save_folder + '/testloader.dat')
    ...
    if not args.resume_model:
        ...
        torch.save(state, 'MNIST/trained_nets/' + save_folder + '/model_0.t7')
        torch.save(opt_state, 'MNIST/trained_nets/' + save_folder + '/opt_state_0.t7')
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch == 1 or epoch % args.save_epoch == 0 or epoch == 150:
            ...
            torch.save(state, 'MNIST/trained_nets/' + save_folder + '/model_' + str(epoch) + '.t7')
            torch.save(opt_state, 'MNIST/trained_nets/' + save_folder + '/opt_state_' + str(epoch) + '.t7')
